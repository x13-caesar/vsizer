...
class Config:
    ...
    def working_folder(self) -> str:
        return self.conf.get("common", "working_folder")
    ...
    def record_megadata(self) -> bool:
        return self.conf.getboolean("megadata", "switch")

    def megadata_file(self) -> str:
        return self.conf.get("megadata", "record_file")

    def check_history_file(self) -> str:
        return self.conf.get("check", "history_file")

    def check_log_file(self) -> str:
        return self.conf.get("check", "log_file")    

    def check_favorites(self) -> str:
        return self.conf.get("check", "favorites")    
    ...
    def _default_config() -> configparser.ConfigParser:
        ...
        conf.set(sec9, "record_file", "AV_Dataset.csv")

        sec10 = 'check'
        conf.add_section(sec10)
        conf.set(sec10, "history_file", "history.csv")
        conf.set(sec10, "log_file", "log.json")
        conf.set(sec10, "favorites", "favorites_genre.csv")
...
if __name__ == "__main__":
    ...
    print(config.record_megadata())
    print(config.megadata_file())
    print(config.check_history_file())
    print(config.check_log_file())
    print(config.check_favorites())
